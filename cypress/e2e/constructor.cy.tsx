...
describe('проверяем функциональность страницы конструктора бургера', () => {
   beforeEach(() => {
        ...
        cy.get('[data-cy=sectionConstructor]').as('sectionConstructor');
    }); 
    ...
    it('Тестируем добавление ингредиентов из списка в конструктор.', () => {
        ...
        cy.addIngredientToConstructor(testBun._id);
        cy.addIngredientToConstructor(testMain._id);
        cy.addIngredientToConstructor(testSauce._id);

        // проверяем, что булка добавилась в оба блока
        cy.get('@sectionConstructor').within(() => {
    ...
    it('Тестируем работу модальных окон: открытие модального окна ингридиента и закрытие по клику на крестик.', () => {
        ...
        cy.get('@modal').should('be.visible').contains(testBun.name).should('be.visible');
